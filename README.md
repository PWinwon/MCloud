...
> 꼭 하나의 답만이 존재하는 것이 아니다

워드 클라우드 구현 과정에서 형태소 분석을 위해 `KoNLPY` 라이브러리를 설치하는 과정에서 수많은 오류를 발견하였습니다. 이를 해결하기 위해 토요일 꼬박 하루를 썼지만 설치하는데에 실패하였습니다. 10시간이 넘게 라이브러리를 설치조차 하지 못한 자신에 좌절감이 들었지만 주저앉지 않았습니다. 라이브러리가 실패했다고 방법이 없는 것이 아니라고 생각했기 때문에 인터넷을 통해 형태소 분석 API 를 찾아 이용하였습니다. API 이용은 키발급받는 30분을 받는 시간을 제외하고는 수월하게 진행하여 극복할 수 있었습니다. 쉬운 방법을 앞에 두고 어려운 방법에 전전긍긍하던 모습에 허탈한 마음보다는 다른 방법을 강구하고 새로운 기술을 터득하는 개발자의 자세를 갖출 수 있었던 계기가 된 것 같습니다.



> 사실은 역할이 의미 없었던 최종 프로젝트..

크게 프론트엔드와 백엔드로 역할을 분담하여 개발을 진행하였지만, 프로젝트 진행 과정에서 그것은 의미가 없다는 것을 느꼈습니다. 하나의 기능과 그 기능을 디자인하는 과정이 항상 딱 맞게 할 일이 생기는 것이 아닌 각자의 것만 하게 된다면 비어있는 시간이 발생하게 되고, 그 시간을 서로의 역할을 다시 분담하여 프로젝트를 수행한 것이 결과물의 완성도에 큰 영향을 미친 것 같습니다. 

특히 내가 백엔드를 맡았다고 해서 디자인은 나몰라라가 아닌 하나의 팀에서 프로젝트의 성공을 목표의식을 가지고 서로의 역할에 관심을 가지고 진행상황을 파악하여 피드백하는 과정이 중요하다는 것을 느꼈습니다. 실제로 디자인에서 큰 시간이 소모하여 저도 다방면의 라이브러리와 여러 웹사이트를 통해 아이디어를 얻고 이를 적용하여 더욱 완성도 높은 결과물이 되었다는 것을 느꼈습니다.

또한 그 과정에서 같은 목표를 향해 가는 팀원과의 소통과 프로젝트관리를 위해 노션을 이용하고 그 과정에서 부족한 부분을 나날이 보완하는 발전하는 저를 돌아볼 수 있었습니다. 이번 프로젝트로 마지막이 아닌 개발자의 꿈을 향해 한 걸음 더 전진한 제 자신을 돌아보며, 다음의 프로젝트에서도 좋은 팀원들과 함께 성장하겠다 다짐하였습니다.
